...
const NAME = process.env.WHATSGPS_NAME
...
async function getToken() {
  ...
  if (data.ret !== 1) throw new Error('Login ret != 1')
  const token = data.data?.token
  const userId = data.data?.userId
  if (!token) throw new Error('No token in login response')
  // No dan expires; cachea por 45 minutos (ajústalo si soporte te da un TTL)
  cache = { token, userId, exp: Date.now() + 45 * 60 * 1000 }
  return token
}

export async function getUserId() {
  // Si ya cacheamos, úsalo
...
export async function getCarsByUser(userId) {
  ...
  const data = await r.json()
  if (data.ret !== 1) throw new Error('getByUserId ret != 1')
  return data.data || []
}
...
export async function getStatusesByUser(userId, { mapType } = {}) {
  ...
  const data = await r.json()
  if (data.ret !== 1) throw new Error('carStatus ret != 1')
  return data.data || []
}
...
export async function getHistory({ carId, startTime, endTime, mapType }) {
  ...
  url.searchParams.set('startTime', startTime) // "2025-09-27 10:00:00" UTC
  url.searchParams.set('endTime', endTime)     // "2025-09-27 11:00:00" UTC
  if (mapType != null) url.searchParams.set('mapType', String(mapType))
  const r = await fetch(url)
  if (!r.ok) throw new Error(`queryHistory ${r.status}`)
  const data = await r.json()
  if (data.ret !== 1) throw new Error('queryHistory ret != 1')
  return data.data || []
}
